    ...
    const fetchData = async () => {
      try {
        ...
        const [schoolsResponse, countriesResponse, typesResponse] = await Promise.all([
          axios.get('http://192.168.0.101:3000/schools'),
          axios.get('http://192.168.0.101:3000/countries_all'),
          axios.get('http://192.168.0.101:3000/school_types_all')
        ]);
  ...
  const navigateToSchoolDetail = async (school) => {
    try {
      ...
      const [countryResponse, typeResponse] = await Promise.all([
        axios.get(`http://192.168.0.101:3000/countries/${school.country_id}`, config),
        axios.get(`http://192.168.0.101:3000/school_types/${school.type_id}`, config)
      ]);
