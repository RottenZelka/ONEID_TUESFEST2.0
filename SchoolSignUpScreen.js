...
const SchoolSignUpScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        const response = await axios.get('http://192.168.0.101:3000/countries');
    ...
    const fetchSchoolTypes = async () => {
      try {
        const response = await axios.get('http://192.168.0.101:3000/school_types');
  ...
  const handleSignUp = async () => {
    try {
      const response = await axios.post('http://192.168.0.101:3000/signupSchool', {
        abbreviation,
        password,
